$(document).ready(function() {
    ...
    function createControls() {
        ...
        var $settings = $('<div>')
            .appendTo($container);
        if (!Boolean(sessionStorage.getItem('hmpoAutoFill-settings'))) $settings.hide();
    ...
    function fill() {
        ...
        if ($button.length) return $button.click();

        var buttonHref = $('a.govuk-button').first().attr('href');
        if (buttonHref) return window.location = buttonHref;
